var DishitemView = function (container, model) {
	...
	for (dish in full_menu){
		if(full_menu[dish].id == current_dish){
			...
			var item_description = model.getDescription(current_dish);
			...
			var ingredient_array = model.getIngredients(current_dish);

			for(ingredient in ingredient_array){

				ing_name += ingredient_array[ingredient].name;
				ing_name += "<br>";
				ing_quantity += ingredient_array[ingredient].quantity * model.getNumberOfGuests();
				ing_quantity += "<br>";
				ing_unit += ingredient_array[ingredient].unit;
				ing_unit += "<br>"
